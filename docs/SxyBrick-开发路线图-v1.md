...
> - ✅ **6.5b OCR**：`src/utils/ocr.js` 纯函数（语言白名单/资源路径/文本归一/空判定/缩放/云端请求构造/响应解析）；`docs-lib.ocrDoc` 状态机（图片直接识别、扫描版 PDF 逐页渲染 2x → 识别）；**本地 Tesseract 优先**（worker + simd-lstm 核心 wasm 本地化 `public/ocr/`，语言包 CDN 缓存）+ **云端 OpenAI 兼容视觉开关**；视图 OCR 按钮/进度/设置面板（语言/云端/语言目录）。
...
## 五、下一步（按质量杠杆）
1. **资料中心 Phase 6 余项**：6.6 联动（知识图谱抽概念 → 错题溯源 → 复习上下文增强）。主线（上传→解析→预览→问答→建卡→OCR）已闭环。
