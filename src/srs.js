...
// 输入：当前 level、ease、自评 rating(0没记住/1还模糊/2记住了)、强度 intensity
// 输出：{ level, ease, intervalDays, dueAt }
...
export function computeNext(card, rating, intensity = 1) {
  const now = Date.now();
  let { level, ease } = card;
  ...
  else if (level <= 4) days = GRADUATED_STEPS[level - 1];
  else days = 15 * Math.pow(ease, level - 4);
