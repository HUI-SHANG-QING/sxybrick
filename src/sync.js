...
import { sweepOrphanRows, repairBrokenDueAt, pruneUserOps, pruneTombstones, pruneAiUsage, prunePrivacyRecords, invalidateFailCountCache, invalidateDashboardCache } from './repo.js';
...
export async function restoreSnapshot(id) {
  ...
  await db.transaction('rw', ...txTables, async () => {
    for (const t of tables) {
      ...
      if (rows.length) { invalidateFailCountCache(); invalidateDashboardCache(); await db[t.table].bulkPut(rows); }
    }
...
export async function importBackup(backup, opts = {}) {
  ...
  for (const t of effTables) {
    ...
    if (toWrite.length) { invalidateFailCountCache(); invalidateDashboardCache(); await db[t.table].bulkPut(toWrite); }
  ...
  // 幽灵卡自愈（force）：备份包 JSON 往返可能把历史 NaN dueAt 变成 null——
  // 导入后强制扫一遍，把损坏行置为「立即到期」，否则它们会再次隐形于复习队列。
  try {
