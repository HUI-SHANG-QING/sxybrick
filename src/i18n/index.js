...
const zh = {
  ...
  agent: {
    orchestrator: {
      ...
      pipelineError: '流水线执行失败（{msg}），已改为单 Agent 处理',
    ...
    llm: {
      ...
      badMessages: 'AI 调用参数有误：messages 必须是数组（形如 [{{role:"user",content:"…"}}]），当前收到 {got}。',
...
const en = {
  ...
  agent: {
    orchestrator: {
      ...
      pipelineError: 'Pipeline failed ({msg}); falling back to single-agent mode',
    ...
    llm: {
      ...
      badMessages: 'Invalid AI call arguments: `messages` must be an array (e.g. [{{role:"user",content:"…"}}]); got {got}.',
