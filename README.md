...
覆盖 FSRS 调度、同步合并语义、校准、预测、净值、apkg 解析、自我解释同步、插件系统（manifest 校验 / Agent 桥接 / 钩子映射 / 插件包 / 官方示例）、资料中心（OPFS 路由 / 解析器 / 建卡草稿 / 文件问答 / 黄金路径）等 255+ 项断言，含 fake-indexeddb 的四条黄金路径集成测试（建卡→复习→到期闭环 / 备份冲突合并 / 删除→墓碑跨设备级联 / 资料上传→解析→索引→检索→确认建卡→删除全链路）。CI 中 `npm test` 作为发布门禁，失败则不构建不发布。
