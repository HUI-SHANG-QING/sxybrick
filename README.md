...
- **知识图谱联动（模块联动第一优先）**：资料解析/OCR 完成后**自动入图**——把资料与它覆盖的卡片（同科目 + 卡片题干前缀命中原文）建立「涵盖」边，资料成为图谱一等节点，可在「知识图谱」页点击资料节点跳回资料库；支持手动「🔗 关联卡片」。卡片可**反查来源资料与原文片段**（错题溯源）；**复习上下文增强**——背诵时翻转卡片后展示来源资料名与原文片段（未翻面不剧透）。
- 数据表：`docFiles`（同步元数据）+ `docTexts`（本地全文），避开旧 `docs` 表；复用 `embeddings`（`sourceType:'doc'`）做检索、`graphEdges`（新增 `docId`/`type`）做资料↔卡片图谱。
